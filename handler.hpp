...
void start_server(SERVER_TYPE &server){
	...
	server.resource["^/string/?$"]["POST"] = [](ostream & response,Request & request){
		//get string from istream(*request.content);
		stringstream ss;
	...
	//proc GET return message of request
	...
	//处理访问/match/[字母+数字组成的字符串]的GET请求，例如执行请求GET /match/abc123,将返回abc123
	...
	server.default_resource["^/?(.*)$"]["GET"]=[](ostream & response, Request & request){
		...
		//简单的平台无关的文件或目录检查
		if(filename.find('.')==string::npos){//没有查找到.
